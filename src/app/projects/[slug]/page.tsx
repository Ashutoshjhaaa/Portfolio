...
import { PROJECTS, BASE_URL } from "@/app/constants/data";
...
export async function generateMetadata({
  ...
  // This WILL use your layout template: "%s | Deepak Modi"
  return {
    title: project.name, // "ResumeGPT" becomes "ResumeGPT | Deepak Modi"
    description: project.description,
    alternates: {
      canonical: `/projects/${slug}`,
    },
    openGraph: {
      title: `${project.name} | Subhash Jha Portfolio`,
      description: project.description,
      url: `${BASE_URL}/projects/${slug}`,
      siteName: "Subhash Jha Portfolio",
      type: "website",
        ...
        <div className="flex gap-3">
          {project.link && (
            <Link
              ...
              className="btn px-4 py-2 text-sm font-semibold border border-border/40 rounded-xl flex items-center gap-2 bg-background/50 hover:bg-hover-bg hover:border-border-hover/60 hover:text-foreground transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
            >
...
const ProjectContent = ({ project }: { project: Project }) => (
  <div className="divide-y divide-border">
    ...
    {project.images?.hero && (
      <div className="p-4">
        <div className="relative aspect-video overflow-hidden rounded-xl border border-border/50 shadow-sm">
          <Image
        ...
        {project.technicalDetails.map(
          (detail: TechnicalDetail, index: number) => (
            <div key={index} className="p-4">
              ...
              {detail.code && (
                <pre className="bg-muted p-4 border border-border text-sm overflow-x-auto">
      ...
      <section className="divide-y divide-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          {project.images.gallery.map((image: ProjectImage, index: number) => (
            <div
              key={index}
              className="relative group p-2 border-border border-b md:odd:border-r last:border-b-0 md:[&:nth-last-child(-n+2)]:border-b-0"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4">
          {Object.entries(project.metrics)
            ...
            .map(([key, value], index, array) => (
              <div
                key={key}
                className={`text-center p-4 border-border border-r border-b 
...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return (
    <div className="font-sans min-h-screen flex flex-col items-center relative px-2 py-6">
      <main className="relative z-10 max-w-3xl w-full mx-auto liquid-glass rounded-2xl overflow-hidden divide-y divide-border">
